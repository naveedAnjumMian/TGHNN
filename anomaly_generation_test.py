...
def anomaly_generation(anomaly_percent, data, user_data, rate_data, user_n, rate_n, time, rate, labels, cfg):
    ...
    user_idx_1 = np.expand_dims(np.transpose(np.random.choice(user_n, int(test.shape[0] * anomaly_percent))),
                                axis=1)  # from [0, n-1]
    user_idx_2 = np.expand_dims(np.transpose(np.random.choice(user_n, int(test.shape[0] * anomaly_percent))),
                                axis=1)  # m * 1
    ...
    user_time = np.expand_dims(np.transpose(np.random.choice(np.unique(time[user_test_idx]), user_fake_edges.shape[0])),
                               axis=1)
    ...
    rate_idx_1 = np.expand_dims(np.transpose(np.random.choice(user_n, int(test.shape[0] * anomaly_percent))),
                                axis=1)  # from [0, n-1]
    rate_idx_2 = np.expand_dims(np.transpose(np.random.choice(rate_n, int(test.shape[0] * anomaly_percent))),
                                axis=1)  # m * 1
    ...
    rate_time = np.expand_dims(np.transpose(np.random.choice(np.unique(time[user_test_idx]), rate_fake_edges.shape[0])),
                               axis=1)
...
def main_test(all_data, cfg):
    ...
    # quit()
    user_ori = np.concatenate((all_data[all_data[:, 2] == 1][:, [0]], all_data[all_data[:, 2] == 1][:, [1]],
                               all_data[all_data[:, 2] == 2][:, [0]]))
