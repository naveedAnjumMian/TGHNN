...
import anomaly_generation_all
import anomaly_generation_test
...
def train(model, optimizer, cfg):
    ...
    if cfg['if_injected_train']:
        ...
        # data = pd.read_csv(f'digg_{cfg["id"]}/injected_all_10%.csv', dtype=np.int64, skiprows=567910, nrows=1114).to_numpy()
        print(data.shape)
        # exit()
    ...
    for epoch in tqdm(range(cfg['epochs'])):
        ...
        tqdm.write(
            'val loss: %f | val_acc: %f | val_auc: %f | val_pr_auc: %f' % (val_loss, val_acc, val_auc, val_pr_auc))
...
if __name__ == "__main__":
    ...
    # print(inters_user.shape)
    # print(np.max(list(user_userToidx.values())) + 1)
    # print(inters_user)
    # exit()
    ...
    if cfg['if_injected_train']:
        ...
        anomaly_generation_all.main_all(all_data, cfg)
    else:
        print('Test Data Anomaly generation......')
        anomaly_generation_test.main_test(all_data, cfg)
